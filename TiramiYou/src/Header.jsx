...
import { Menu, X, ShoppingCart, MapPin, Briefcase, Home as HomeIcon, Cake, Award, Users, Newspaper, Phone } from 'lucide-react';
    ...
    <>
      <nav className={`fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-50 transition-all duration-300 ${
        scrolled 
          ? 'shadow-2xl py-3' 
          : 'shadow-xl py-4'
      } backdrop-blur-md rounded-full`} style={{ backgroundColor: '#FF93A2', borderColor: '#FF93A2', borderWidth: '1px', borderStyle: 'solid' }}>
            ...
            <div className="hidden lg:flex items-center gap-6 xl:gap-8">
              {navItems.map((item) => (
                ...
                >
                  ...
                  {item.name}
                  <span className="absolute -bottom-1 left-0 w-0 h-1 rounded-full group-hover:w-full transition-all duration-300" style={{ backgroundColor: '#FEF6CC' }}></span>
                </a>
              ))}
            </div>
